...
# ==========================================
# 1. Config & API Setup (ซิงค์ตรงจาก app.py)
# ==========================================
...
with st.spinner("⏳ กำลังรวบรวมข้อมูลพอร์ตรวม..."):
    ...
    if all_holdings:
        ...
        # สร้าง DataFrame แต่ละโบรกเกอร์
        portfolio_rows = []
        for index, row in df_raw.iterrows():
            sym = row['Symbol']
            qty = row['Qty']
...
with tab_all:
    ...
    if not df_port.empty:
        grand_invested = df_port['Invested_USD'].sum()
        grand_market = df_port['Market_Value_USD'].sum()
        grand_pnl = grand_market - grand_invested
        grand_pnl_pct = (grand_pnl / grand_invested * 100) if grand_invested > 0 else 0
        
        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown(f'<div class="metric-card"><div class="metric-label">💵 เงินลงทุนรวมทั้งสิ้น</div><div class="metric-value">${grand_invested:,.2f}</div></div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div class="metric-card"><div class="metric-label">📈 มูลค่าตลาดรวมพอร์ตทั้งหมด</div><div class="metric-value">${grand_market:,.2f}</div></div>', unsafe_allow_html=True)
        with c3:
            pnl_class = "text-green" if grand_pnl >= 0 else "text-red"
            pnl_prefix = "+" if grand_pnl >= 0 else ""
            st.markdown(f'<div class="metric-card"><div class="metric-label">📊 กำไร / ขาดทุนสุทธิรวม</div><div class="metric-value {pnl_class}">{pnl_prefix}${grand_pnl:,.2f} ({grand_pnl_pct:+.2f}%)</div></div>', unsafe_allow_html=True)
        ...
        with col_g1:
            ...
            fig1 = px.pie(df_broker_summary, names="Broker", values="Market_Value_USD", title="สัดส่วนมูลค่าพอร์ตแยกตามโบรกเกอร์", hole=0.4, template="plotly_dark")
            st.plotly_chart(fig1, use_container_width=True)

        with col_g2:
            df_symbol_summary = df_port.groupby("Symbol")["Market_Value_USD"].sum().reset_index()
            fig2 = px.pie(df_symbol_summary, names="Symbol", values="Market_Value_USD", title="สัดส่วนการถือครองหุ้นทุกตัวในพอร์ต", template="plotly_dark")
...
with tab_webull:
    ...
    if not df_w.empty:
        st.dataframe(df_w[["Symbol", "Qty", "Cost", "Price", "Invested_USD", "Market_Value_USD", "PnL_USD", "PnL_Pct"]].rename(columns={
            "Cost": "Avg Cost ($)", "Price": "Market Price ($)", "Invested_USD": "Total Cost ($)", "Market_Value_USD": "Market Value ($)", "PnL_USD": "Unrealized P/L ($)", "PnL_Pct": "P/L (%)"
        }).style.format({
            "Qty": "{:,.4f}", "Avg Cost ($)": "${:,.2f}", "Market Price ($)": "${:,.2f}", "Total Cost ($)": "${:,.2f}", "Market Value ($)": "${:,.2f}", "Unrealized P/L ($)": "${:+,.2f}", "P/L (%)": "{:+.2f}%"
...
with tab_dime_us:
    ...
    if not df_dus.empty:
        st.dataframe(df_dus[["Symbol", "Qty", "Cost", "Price", "Invested_USD", "Market_Value_USD", "PnL_USD", "PnL_Pct"]].rename(columns={
            "Cost": "Avg Cost ($)", "Price": "Market Price ($)", "Invested_USD": "Total Cost ($)", "Market_Value_USD": "Market Value ($)", "PnL_USD": "Unrealized P/L ($)", "PnL_Pct": "P/L (%)"
        }).style.format({
            "Qty": "{:,.4f}", "Avg Cost ($)": "${:,.2f}", "Market Price ($)": "${:,.2f}", "Total Cost ($)": "${:,.2f}", "Market Value ($)": "${:,.2f}", "Unrealized P/L ($)": "${:+,.2f}", "P/L (%)": "{:+.2f}%"
        }), use_container_width=True)
...
with tab_dime_th:
    ...
    if not df_dth.empty:
        # คำนวณกลับเป็น THB สำหรับแสดงผลในแท็บหุ้นไทย
        ...
        st.dataframe(df_dth_disp[["Symbol", "Qty", "Cost", "Price", "Total_Cost_THB", "Market_Value_THB", "PnL_THB", "PnL_Pct"]].rename(columns={
            "Cost": "Avg Cost (฿)", "Price": "Market Price (฿)", "Total_Cost_THB": "Total Cost (฿)", "Market_Value_THB": "Market Value (฿)", "PnL_THB": "Unrealized P/L (฿)", "PnL_Pct": "P/L (%)"
        }).style.format({
            "Qty": "{:,.0f}", "Avg Cost (฿)": "฿{:,.2f}", "Market Price (฿)": "฿{:,.2f}", "Total Cost (฿)": "฿{:,.2f}", "Market Value (฿)": "฿{:,.2f}", "Unrealized P/L (฿)": "฿{:+,.2f}", "P/L (%)": "{:+.2f}%"
        }), use_container_width=True)
    else:
