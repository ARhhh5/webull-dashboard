...
# ฟังก์ชันไฮไลต์สีสำหรับช่องกำไร/ขาดทุน
...
with st.spinner("⏳ กำลังดึงราคาสดหุ้นไทย..."):
    ...
    if holdings:
        ...
        df_display = df.copy()
        df_display["กำไร/ขาดทุน (THB)"] = df_display["PnL"]
        
        st.dataframe(
            df_display[["หุ้น TH", "จำนวนหุ้น", "ต้นทุนเฉลี่ย", "ราคาตลาด", "เงินลงทุน (THB)", "มูลค่าปัจจุบัน (THB)", "กำไร/ขาดทุน (THB)"]]
            .style.map(color_pnl_text, subset=["กำไร/ขาดทุน (THB)"])
            .format({
                "จำนวนหุ้น": "{:,.2f}",
                "ต้นทุนเฉลี่ย": "฿{:,.2f}",
                "ราคาตลาด": "฿{:,.2f}",
