...
st.markdown("""
    ...
    .pnl-positive { color: #00e676 !important; }
    .pnl-negative { color: #ff5252 !important; }
    </style>
...
# ฟังก์ชันใส่สี Background และตัวหนังสือใน DataFrame
def style_pnl(val):
    ...
    if holdings:
        ...
        for h in holdings:
            ...
            try:
                ...
                # ใช้ history เป็นหลักเพราะยิงดึงราคาหุ้นไทยใน yfinance เสถียรที่สุด
                hist = ticker.history(period="5d")
                if not hist.empty:
                    price = float(hist['Close'].iloc[-1])
                else:
                    # ถ้าดึงไม่ได้จริงๆ ลอง fast_info
                    price = float(ticker.fast_info.get('last_price', 0.0))
            except Exception:
                price = 0.0
            
            # กรณีดึงราคาไม่สำเร็จ ให้ใช้ cost แก้ขัด แต่ออกเตือน
            if price == 0.0:
                price = cost
                st.warning(f"⚠️ ไม่สามารถดึงราคาล่าสุดของ {sym} ได้ ใช้ราคาต้นทุนแทน")
            ...
            data.append({
                ...
                "PnL_Raw": pnl,
                "กำไร/ขาดทุน": f"{'+' if pnl > 0 else ''}฿{pnl:,.2f} ({pnl_pct:+.2f}%)"
            })
        ...
        # แสดงผลตารางพร้อมการใส่สี
        st.dataframe(
            df[["หุ้น TH", "จำนวนหุ้น", "ต้นทุนเฉลี่ย", "ราคาตลาด", "เงินลงทุน (THB)", "มูลค่าปัจจุบัน (THB)", "กำไร/ขาดทุน"]]
            .style.map(style_pnl, subset=["PnL_Raw"]) # ใช้คอลัมน์ซ่อนช่วยคำนวณสี
            .format({
                "จำนวนหุ้น": "{:,.2f}",
                "ต้นทุนเฉลี่ย": "฿{:,.2f}",
                "ราคาตลาด": "฿{:,.2f}",
