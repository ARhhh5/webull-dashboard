...
# ฟังก์ชันใส่สี Background + ตัวอักษรให้เห็นชัดเจนตามเครื่องหมาย + หรือ -
def style_pnl_str(val):
    s = str(val).strip()
    if s.startswith("+"):
...
with st.spinner("⏳ กำลังดึงราคาสดหุ้นไทย..."):
    ...
    if holdings:
        ...
        for h in holdings:
            ...
            # ฟอร์แมตข้อความกำไร/ขาดทุนพร้อมเครื่องหมาย
            if pnl > 0:
        ...
        # แสดงผล DataFrame
        display_cols = ["หุ้น TH", "จำนวนหุ้น", "ต้นทุนเฉลี่ย", "ราคาตลาด", "เงินลงทุน (THB)", "มูลค่าปัจจุบัน (THB)", "กำไร/ขาดทุน"]
        
        st.dataframe(
            df[display_cols]
            .style.map(style_pnl_str, subset=["กำไร/ขาดทุน"])
            .format({
                "จำนวนหุ้น": "{:,.2f}",
                "ต้นทุนเฉลี่ย": "฿{:,.2f}",
                "ราคาตลาด": "฿{:,.2f}",
