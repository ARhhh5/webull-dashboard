...
st.title("🧠 AI Fundamental Analyzer Prompt Generator")
st.markdown("เครื่องมือเจนฯ Prompt วิเคราะห์ปัจจัยพื้นฐานหุ้นแบบเจาะลึก 12 หัวข้อ สำหรับนำไปใช้กับ Gemini, ChatGPT หรือ Claude")
st.markdown("---")

# ==========================================
# 1. ส่วนรับข้อมูล Ticker จากผู้ใช้
# ==========================================
...
with col_input:
    ticker_input = st.text_input(
        "พิมพ์ชื่อ Ticker หุ้น (เช่น NVDA, MU, TSLA, Apple, PTT):",
        value="",
        placeholder="ตัวอย่าง: NVDA"
    )

with col_info:
    st.info("💡 **คำแนะนำ:** นำ Prompt ที่ได้ไปวางใน AI รุ่นพรีเมียม (เช่น ChatGPT Plus, Claude 3.5 Sonnet, Gemini Advanced) เพื่อผลลัพธ์ข้อมูลที่แม่นยำและลึกที่สุดครับ")

# ==========================================
# 2. แม่แบบ Prompt (12 หัวข้อ)
# ==========================================
...
# ==========================================
# 3. แสดงผล Prompt และสร้างกล่อง Copy
# ==========================================
if ticker_input.strip():
    clean_ticker = ticker_input.strip().upper()
    generated_prompt = PROMPT_TEMPLATE.format(ticker=clean_ticker)
    
    st.subheader(f"📋 Prompt พร้อมใช้งานสำหรับหุ้น: {clean_ticker}")
    st.caption("คลิกที่มุมขวาบนของกล่องโค้ดด้านล่างเพื่อก๊อปปี้ไปวางใน AI ได้เลยครับ")
    
    st.code(generated_prompt, language="text")
else:
    st.info("👈 พิมพ์ชื่อ Ticker หุ้นในช่องด้านบน แล้ว Prompt วิเคราะห์งบการเงินฉบับเต็มจะปรากฏขึ้นที่นี่ทันทีครับ!")
