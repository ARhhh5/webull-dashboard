...
# ==========================================
st.set_page_config(page_title="AI Fundamental Analyzer - GOD MODE", layout="wide")
...
# Minimal Header
st.markdown('<div class="page-title-minimal">🧠 AI Fundamental Analyzer Engine (GOD MODE)</div>', unsafe_allow_html=True)
st.markdown('<div class="page-subtitle-minimal">Global Growth Fund OS + Lazy Long-term Holder Framework (Gemini 2.5 Flash)</div>', unsafe_allow_html=True)

# ==========================================
# 2. PROMPT TEMPLATES (GOD MODE & 7 TOOLS)
# ==========================================
...
หุ้นที่ต้องการให้วิเคราะห์คือ Ticker: {ticker}
...
# 7 เครื่องมือวิเคราะห์เสริม Prompts
TOOL_PROMPTS = {
    "1️⃣ สแกนสุขภาพการเงินบริษัท": "ช่วยอ่านงบการเงินล่าสุดของหุ้น {ticker} แล้วสรุป: สุขภาพโดยรวม, แนวโน้มรายได้-กำไร, ภาระหนี้+กระแสเงินสด, จุดเด่น 3 ข้อ, Red flags 3 ข้อ, และคำถามที่ควรหาคำตอบเพิ่ม",
    ...
    "7️⃣ กรอบช่วยตัดสินใจ ซื้อ/ขาย/ถือ": "ตั้งคำถามย้อน 5 ข้อสำหรับหุ้น {ticker} -> แนะนำการแบ่งไม้ -> กำหนดเงื่อนไข ซื้อ/ขาย/ตัดขาดทุน -> และคำเตือนด้านอารมณ์ในการเทรด"
}

# ==========================================
# 3. INPUT CONTROL & SECRETS
# ==========================================
st.markdown("### 🔎 ระบุ Ticker หุ้นที่ต้องการวิเคราะห์")
col_input, col_space = st.columns([2.5, 2.5])

with col_input:
    ticker_input = st.text_input(
        "พิมพ์ชื่อ Ticker หุ้น (เช่น NVDA, TSLA, AAPL, PTT, CPN):",
        value="",
...
# ==========================================
if "ai_god_mode" not in st.session_state:
    st.session_state["ai_god_mode"] = "FULL_GOD"

nav_mode = st.session_state["ai_god_mode"]

c_b1, c_b2, c_b3 = st.columns([1.5, 1.5, 1.5])

with c_b1:
    b1_type = "primary" if nav_mode == "FULL_GOD" else "secondary"
    if st.button("👑 GOD MODE Full Scan", key="btn_god_full", type=b1_type, use_container_width=True):
        st.session_state["ai_god_mode"] = "FULL_GOD"
        st.rerun()

with c_b2:
    b2_type = "primary" if nav_mode == "EXTRA_TOOLS" else "secondary"
    if st.button("🛠️ 7 เครื่องมือวิเคราะห์เสริม", key="btn_god_tools", type=b2_type, use_container_width=True):
        st.session_state["ai_god_mode"] = "EXTRA_TOOLS"
        st.rerun()

with c_b3:
    b3_type = "primary" if nav_mode == "RAW_PROMPT" else "secondary"
    if st.button("📋 ก๊อปปี้ Prompt สำเร็จรูป", key="btn_god_prompt", type=b3_type, use_container_width=True):
        st.session_state["ai_god_mode"] = "RAW_PROMPT"
        st.rerun()
...
if nav_mode == "FULL_GOD":
    if not HAS_GENAI:
        ...
        if not gemini_api_key or gemini_api_key == "XXXXX":
            ...
            if st.button(f"🚀 เริ่มการวิเคราะห์ GOD MODE สแกนหุ้น {clean_ticker}", type="primary", use_container_width=True):
                with st.spinner(f"⏳ โอเลี้ยง GOD MODE กำลังประมวลผลวิเคราะห์เชิงลึก {clean_ticker} กรุณารอสักครู่..."):
                    try:
                        ...
                        st.markdown(f"## 🏆 GOD MODE Analysis Report: **{clean_ticker}**")
                        st.markdown("---")
                        st.markdown(response.text)
                        st.markdown('</div>', unsafe_allow_html=True)
                        st.success("✅ ประมวลผลวิเคราะห์ GOD MODE เสร็จสิ้น!")
                    except Exception as e:
                        st.error(f"❌ เกิดข้อผิดพลาดในการเรียกใช้ AI: {str(e)}")
    else:
        st.info("👈 พิมพ์ชื่อ Ticker หุ้นด้านบน แล้วกดปุ่มเพื่อเริ่มวิเคราะห์ GOD MODE ได้ทันทีครับ!")
    ...
    if clean_ticker:
        ...
        st.info("👈 พิมพ์ชื่อ Ticker หุ้นด้านบนก่อนใช้งานเครื่องมือเสริมครับ")
